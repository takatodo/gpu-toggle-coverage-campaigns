#!/usr/bin/env python3
"""
Summarize the next family-expansion axis after the Vortex first surface has
been accepted.
"""
...
DEFAULT_JSON_OUT = REPO_ROOT / "work" / "campaign_post_vortex_axes.json"
...
def _runner_support_for_family(runners_dir: Path, family_name: str) -> dict[str, Any]:
    ...
    trio_count = sum(int(path.exists()) for path in (stock_runner, baseline_runner, comparison_runner))
    if trio_count == 3:
        return {
            "kind": "campaign_trio",
            "score": 3,
...
def _readiness_support_for_family(readiness_dir: Path, family_name: str) -> dict[str, Any]:
    ...
    elif "ready_for_gpu_toggle" in md_text or "tier2_abc_completed" in md_text or md_path.is_file():
        readiness_score = 1
...
def _candidate_rows(*, designs_root: Path, runners_dir: Path, readiness_dir: Path) -> list[dict[str, Any]]:
    ...
    for tb_path in sorted(designs_root.glob("*/**/*_gpu_cov_tb.sv")):
        ...
        if family_name in {"OpenTitan", "XuanTie", "VeeR", "OpenPiton", "BlackParrot", "Vortex", "XiangShan"}:
            continue
...
def build_axes(
    *,
    vortex_acceptance_payload: dict[str, Any],
    designs_root: Path,
    runners_dir: Path,
    readiness_dir: Path,
) -> dict[str, Any]:
    acceptance_outcome = dict(vortex_acceptance_payload.get("outcome") or {})
    rows = _candidate_rows(designs_root=designs_root, runners_dir=runners_dir, readiness_dir=readiness_dir)
    recommended_family = rows[0]["repo_family"] if rows else None
    fallback_family = rows[1]["repo_family"] if len(rows) > 1 else None

    if str(acceptance_outcome.get("status") or "") != "accepted_selected_vortex_first_surface_step":
        decision = {
            "status": "blocked_vortex_not_yet_accepted",
            "reason": "the_next_post_vortex_family_decision_requires_the_Vortex_first_surface_to_be_accepted_first",
            "recommended_next_task": acceptance_outcome.get("next_action") or "accept_selected_vortex_first_surface_step",
        }
    elif rows:
        decision = {
            "status": "decide_open_next_family_after_vortex_acceptance",
            "reason": "Vortex_is_now_accepted_so_the_next_question_is_the_next_non_Vortex_family_with_gpu_cov_inventory",
        ...
        decision = {
            "status": "revisit_blocked_or_historical_branches_after_vortex_acceptance",
            "reason": "no_additional_post_vortex_family_candidates_are_available_in_the_current_inventory",
            "recommended_next_task": "revisit_blocked_or_historical_branches",
        }

    return {
        "schema_version": 1,
        "scope": "campaign_post_vortex_axes",
        "accepted_vortex_first_surface": {
            "status": acceptance_outcome.get("status"),
            "selected_design": acceptance_outcome.get("selected_design"),
            "selected_vortex_branch_profile_name": acceptance_outcome.get("selected_vortex_branch_profile_name"),
            "selected_vortex_policy_profile_name": acceptance_outcome.get("selected_vortex_policy_profile_name"),
