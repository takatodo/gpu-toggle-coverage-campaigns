#!/usr/bin/env python3
"""
Apply a named XuanTie breadth acceptance profile and regenerate the active gate.
"""
...
DEFAULT_GATE_JSON = REPO_ROOT / "work" / "campaign_xuantie_breadth_acceptance_gate.json"
...
def apply_profile(
    *,
    profile_name: str,
    profiles_dir: Path,
    real_goal_acceptance_json: Path,
    breadth_gate_json: Path,
    selection_path: Path,
    gate_json_path: Path,
) -> dict[str, Any]:
    ...
    gate_payload = build_gate(
        real_goal_acceptance_payload=_read_json(real_goal_acceptance_json),
        breadth_gate_payload=_read_json(breadth_gate_json),
    ...
    return {
        "schema_version": 1,
        "scope": "set_campaign_xuantie_breadth_acceptance",
...
def main() -> int:
    ...
    payload = apply_profile(
        ...
        real_goal_acceptance_json=args.real_goal_acceptance_json.resolve(),
        breadth_gate_json=args.breadth_gate_json.resolve(),
