#!/usr/bin/env python3
"""
Evaluate named post-E906 non-OpenTitan breadth profiles independently of the
...
from audit_campaign_non_opentitan_breadth_gate import build_gate


SCRIPT_DIR = Path(__file__).resolve().parent
REPO_ROOT = SCRIPT_DIR.parent.parent
DEFAULT_BREADTH_AXES_JSON = REPO_ROOT / "work" / "campaign_non_opentitan_breadth_axes.json"
DEFAULT_PROFILES_DIR = REPO_ROOT / "config" / "campaign_non_opentitan_breadth" / "profiles"
DEFAULT_SELECTION_CONFIG = REPO_ROOT / "config" / "campaign_non_opentitan_breadth" / "selection.json"
DEFAULT_JSON_OUT = REPO_ROOT / "work" / "campaign_non_opentitan_breadth_profiles.json"
...
def build_profiles_matrix(
    *,
    ...
    for profile in profiles:
        ...
        if status in {"continue_same_family_ready", "open_fallback_family_ready"}:
            ready_profiles.append(name)
            if recommended_profile_name is None:
                recommended_profile_name = name
        elif status == "hold_post_e906_branch":
    ...
    recommended_decision_axis = "repair_non_opentitan_breadth_profiles"
    if ready_profiles:
        if len(ready_profiles) > 1:
            recommended_decision_axis = "choose_named_non_opentitan_breadth_profile"
        elif current_profile_classification == "ready" and current_profile_name == recommended_profile_name:
            recommended_decision_axis = "accept_current_non_opentitan_breadth_profile"
        else:
            recommended_decision_axis = "choose_named_non_opentitan_breadth_profile"

    return {
        "schema_version": 1,
        "scope": "campaign_non_opentitan_breadth_profiles",
        ...
        "summary": {
            ...
            "ready_profile_names": ready_profiles,
