#!/usr/bin/env python3
"""
Apply a named checkpoint/seed acceptance profile and regenerate the active gate.
"""
...
DEFAULT_GATE_JSON = REPO_ROOT / "work" / "campaign_real_goal_acceptance_gate.json"
...
def apply_profile(
    *,
    ...
    gate_payload = build_gate(
        checkpoint_payload=_read_json(checkpoint_json),
        seed_payload=_read_json(seed_status_json),
    ...
    return {
        "schema_version": 1,
        "scope": "set_campaign_real_goal_acceptance",
...
def main() -> int:
    ...
    payload = apply_profile(
        ...
        checkpoint_json=args.checkpoint_json.resolve(),
        seed_status_json=args.seed_status_json.resolve(),
