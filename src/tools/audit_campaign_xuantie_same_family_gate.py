#!/usr/bin/env python3
"""
Resolve the currently selected XuanTie same-family profile against the current
same-family step summary.
"""
...
DEFAULT_JSON_OUT = REPO_ROOT / "work" / "campaign_xuantie_same_family_gate.json"
...
def _resolve_profile_payload(
    *,
    ...
    if not profile_path.is_file():
        raise FileNotFoundError(f"campaign XuanTie same-family profile not found: {profile_path}")
...
def _build_outcome(*, profile_payload: dict[str, Any], step_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    best_ready_threshold_value = step_outcome.get("candidate_threshold_value") or step_selected_design.get(
        "best_ready_threshold_value"
    )

    if step_design and selected_design and step_design != selected_design:
        return {
            "status": "blocked_selected_design_mismatch",
            "reason": "selected_profile_targets_a_different_same_family_design_than_the_current_step_artifact",
            "next_action": "recompute_same_family_step_or_fix_profile",
    ...
    if step_mode == "default_gate_hold":
        if step_status in {
            ...
            "selected_same_family_design_default_gate_unresolved",
        }:
            return {
                "status": "default_gate_hold",
                "reason": "selected_profile_keeps_the_same_family_design_on_the_current_unresolved_default_gate",
                ...
                "default_comparison_path": step_selected_design.get("default_comparison_path"),
            }
        if step_status == "ready_to_accept_selected_same_family_design":
            return {
                "status": "default_gate_ready",
                "reason": "the_selected_same_family_design_already_has_a_ready_default_gate_line",
        ...
        return {
            ...
            "next_action": step_outcome.get("next_action") or "repair_same_family_default_gate_line",
            "selected_design": step_design or selected_design or None,
        }

    if step_mode == "candidate_only_threshold":
        if (
            step_status == "decide_selected_same_family_design_candidate_only_vs_new_default_gate"
            and best_ready_threshold_value == candidate_threshold_value
        ):
            return {
                "status": "candidate_only_ready",
                "reason": "selected_profile_accepts_the_checked_in_same_family_candidate_only_hybrid_win",
                "next_action": "accept_selected_same_family_candidate_only_step",
                "selected_design": step_design or selected_design or None,
                "comparison_path": step_outcome.get("comparison_path"),
                "speedup_ratio": step_outcome.get("speedup_ratio"),
        ...
        return {
            ...
            "reason": "selected_profile_requires_a_checked_in_same_family_candidate_only_line_that_is_not_currently_available",
            "next_action": step_outcome.get("next_action") or "repair_same_family_candidate_only_line",
            "selected_design": step_design or selected_design or None,
            "candidate_threshold_value": candidate_threshold_value,
        }
    ...
    return {
        ...
        "next_action": "fix_campaign_xuantie_same_family_profile",
    }
...
def build_gate(*, step_payload: dict[str, Any], selection_payload: dict[str, Any], selection_path: Path) -> dict[str, Any]:
    ...
    return {
        "schema_version": 1,
        "scope": "campaign_xuantie_same_family_gate",
        ...
        "context": {
            "active_branch_profile_name": step_context.get("active_branch_profile_name"),
            "active_branch_status": step_context.get("active_branch_status"),
            "selected_seed_design": step_context.get("selected_seed_design"),
            "selected_breadth_design": step_context.get("selected_breadth_design"),
            "branch_recommended_first_design": step_context.get("branch_recommended_first_design"),
        },
        "outcome": outcome,
    }
