#!/usr/bin/env python3
"""
Apply a named XuanTie-C910 runtime profile and regenerate the active gate.
"""
...
DEFAULT_GATE_JSON = REPO_ROOT / "work" / "campaign_xuantie_c910_runtime_gate.json"
...
def apply_profile(
    *,
    ...
    same_family_next_axes_json: Path,
    selection_path: Path,
    gate_json_path: Path,
) -> dict[str, Any]:
    ...
    gate_payload = build_gate(
        runtime_status_payload=_read_json(runtime_status_json),
        same_family_next_axes_payload=_read_json(same_family_next_axes_json),
    ...
    return {
        "schema_version": 1,
        "scope": "set_campaign_xuantie_c910_runtime",
...
def main() -> int:
    ...
    payload = apply_profile(
        ...
        runtime_status_json=args.runtime_status_json.resolve(),
        same_family_next_axes_json=args.same_family_next_axes_json.resolve(),
