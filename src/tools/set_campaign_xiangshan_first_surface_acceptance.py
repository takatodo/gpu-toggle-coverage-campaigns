#!/usr/bin/env python3
"""
Apply a named XiangShan first-surface acceptance profile and regenerate the
...
from audit_campaign_xiangshan_first_surface_acceptance_gate import build_gate
...
DEFAULT_PROFILES_DIR = REPO_ROOT / "config" / "campaign_xiangshan_first_surface_acceptance" / "profiles"
DEFAULT_SELECTION_CONFIG = REPO_ROOT / "config" / "campaign_xiangshan_first_surface_acceptance" / "selection.json"
DEFAULT_GATE_JSON = REPO_ROOT / "work" / "campaign_xiangshan_first_surface_acceptance_gate.json"
...
def _load_profile(*, profiles_dir: Path, profile_name: str) -> tuple[Path, dict[str, Any]]:
    ...
    if not profile_path.is_file():
        raise FileNotFoundError(
            f"campaign XiangShan first-surface acceptance profile not found: {profile_path}"
        )
...
def apply_profile(
    *,
    profile_name: str,
    profiles_dir: Path,
    vortex_gate_json: Path,
    xiangshan_gate_json: Path,
    selection_path: Path,
    gate_json_path: Path,
) -> dict[str, Any]:
    ...
    gate_payload = build_gate(
        vortex_gate_payload=_read_json(vortex_gate_json),
        xiangshan_gate_payload=_read_json(xiangshan_gate_json),
    ...
    return {
        "schema_version": 1,
        "scope": "set_campaign_xiangshan_first_surface_acceptance",
...
def main() -> int:
    ...
    payload = apply_profile(
        ...
        vortex_gate_json=args.vortex_gate_json.resolve(),
        xiangshan_gate_json=args.xiangshan_gate_json.resolve(),
