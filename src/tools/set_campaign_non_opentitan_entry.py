#!/usr/bin/env python3
"""
Apply a named non-OpenTitan entry profile and regenerate the active gate.
"""
...
DEFAULT_GATE_JSON = REPO_ROOT / "work" / "campaign_non_opentitan_entry_gate.json"
...
def apply_profile(
    *,
    ...
    profiles_dir: Path,
    entry_json: Path,
    entry_readiness_json: Path,
    override_candidates_json: Path,
    selection_path: Path,
    gate_json_path: Path,
) -> dict[str, Any]:
    ...
    gate_payload = build_gate(
        entry_payload=_read_json(entry_json),
        readiness_payload=_read_json(entry_readiness_json),
        override_payload=_read_json(override_candidates_json),
    ...
    return {
        "schema_version": 1,
        "scope": "set_campaign_non_opentitan_entry",
...
def main() -> int:
    ...
    payload = apply_profile(
        ...
        entry_json=args.entry_json.resolve(),
        entry_readiness_json=args.entry_readiness_json.resolve(),
        override_candidates_json=args.override_candidates_json.resolve(),
