#!/usr/bin/env python3
"""
Summarize the next breadth axis after the selected VeeR first surface has been
accepted.
"""
...
DEFAULT_VEER_ACCEPTANCE_JSON = REPO_ROOT / "work" / "campaign_veer_first_surface_acceptance_gate.json"
DEFAULT_JSON_OUT = REPO_ROOT / "work" / "campaign_veer_next_axes.json"
...
def build_axes(*, veer_fallback_candidates_payload: dict[str, Any], veer_acceptance_payload: dict[str, Any]) -> dict[str, Any]:
    acceptance_outcome = dict(veer_acceptance_payload.get("outcome") or {})
    ...
    remaining_veer_designs = [design for design in ready_candidates if design != accepted_design]

    if str(acceptance_outcome.get("status") or "") != "accepted_selected_veer_first_surface_step":
        decision_payload = {
            "status": "blocked_selected_veer_first_surface_step_not_accepted",
            "reason": "the_next_VeeR_same-family decision requires the selected VeeR first surface to be accepted first",
            "recommended_next_task": acceptance_outcome.get("next_action") or "accept_selected_veer_first_surface_step",
        }
    elif remaining_veer_designs:
        decision_payload = {
            "status": "decide_continue_to_remaining_veer_design",
            "reason": "the_selected_VeeR_first_surface_is_accepted_so_the_next_question_is_the_next_remaining_VeeR_design",
...
def main() -> int:
    ...
    parser.add_argument("--veer-acceptance-json", type=Path, default=DEFAULT_VEER_ACCEPTANCE_JSON)
    parser.add_argument("--json-out", type=Path, default=DEFAULT_JSON_OUT)
    args = parser.parse_args()

    payload = build_axes(
        veer_fallback_candidates_payload=_read_json(args.veer_fallback_candidates_json.resolve()),
        veer_acceptance_payload=_read_json(args.veer_acceptance_json.resolve()),
    )
