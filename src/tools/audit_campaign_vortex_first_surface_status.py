#!/usr/bin/env python3
"""
Summarize the current Vortex first-surface state after BlackParrot baseline loss
selects Vortex as the next non-XiangShan family.
"""
...
DEFAULT_AXES_JSON = REPO_ROOT / "work" / "campaign_post_blackparrot_axes.json"
DEFAULT_BOOTSTRAP_JSON = REPO_ROOT / "work" / "vortex_gpu_cov_stock_verilator_cc_bootstrap.json"
DEFAULT_BASELINE_JSON = REPO_ROOT / "output" / "validation" / "vortex_cpu_baseline_validation.json"
DEFAULT_META_JSON = REPO_ROOT / "work" / "vl_ir_exp" / "vortex_gpu_cov_vl" / "vl_batch_gpu.meta.json"
DEFAULT_BUILD_LOG = REPO_ROOT / "work" / "vortex_build_vl_gpu.log"
DEFAULT_JSON_OUT = REPO_ROOT / "work" / "campaign_vortex_first_surface_status.json"
...
def build_status(
    *,
    ...
    module_format = str((meta_payload or {}).get("cuda_module_format") or "missing")

    if (
        decision.get("recommended_family") == "Vortex"
        and bootstrap_status == "ok"
        and baseline_status == "ok"
        and meta_payload is not None
    ):
        outcome = {
            "status": "ready_to_finish_vortex_first_trio",
            "reason": "vortex_bootstrap_cpu_baseline_and_gpu_module_artifacts_are_ready",
        ...
        outcome = {
            ...
            "reason": (
                "vortex_bootstrap_and_cpu_baseline_are_ready_but_gpu_codegen_fails_inside_llc_"
                "while_lowering_the_verilated_thread_local_state_symbol"
            ),
            "next_action": (
                "choose_between_offline_vortex_tls_lowering_debug_and_reopening_"
                "xiangshan_fallback_family"
            ),
        }
    else:
        outcome = {
            "status": "bootstrap_or_repair_vortex_first_surface",
            "reason": "vortex_does_not_yet_have_the_expected_bootstrap_baseline_and_gpu_build_state",
            "next_action": "repair_the_missing_vortex_runtime_or_build_prerequisites",
        }

    return {
        "schema_version": 1,
        "scope": "campaign_vortex_first_surface_status",
        "upstream_axes": {
