#!/usr/bin/env python3
"""
Apply a named final VeeR same-family acceptance profile and regenerate the
active gate.
"""
...
from audit_campaign_veer_final_same_family_acceptance_gate import build_gate
...
DEFAULT_PROFILES_DIR = REPO_ROOT / "config" / "campaign_veer_final_same_family_acceptance" / "profiles"
DEFAULT_SELECTION_CONFIG = REPO_ROOT / "config" / "campaign_veer_final_same_family_acceptance" / "selection.json"
DEFAULT_GATE_JSON = REPO_ROOT / "work" / "campaign_veer_final_same_family_acceptance_gate.json"
...
def apply_profile(
    *,
    profile_name: str,
    profiles_dir: Path,
    veer_same_family_acceptance_json: Path,
    veer_final_same_family_gate_json: Path,
    selection_path: Path,
    gate_json_path: Path,
) -> dict[str, Any]:
    ...
    gate_payload = build_gate(
        veer_same_family_acceptance_payload=_read_json(veer_same_family_acceptance_json),
        veer_final_same_family_gate_payload=_read_json(veer_final_same_family_gate_json),
    ...
    return {
        "schema_version": 1,
        "scope": "set_campaign_veer_final_same_family_acceptance",
...
def main() -> int:
    ...
    payload = apply_profile(
        ...
        veer_same_family_acceptance_json=args.veer_same_family_acceptance_json.resolve(),
        veer_final_same_family_gate_json=args.veer_final_same_family_gate_json.resolve(),
