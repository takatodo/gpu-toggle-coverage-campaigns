#!/usr/bin/env python3
"""
Apply a named OpenPiton first-surface acceptance profile and regenerate the
...
from audit_campaign_openpiton_first_surface_acceptance_gate import build_gate
...
DEFAULT_PROFILES_DIR = REPO_ROOT / "config" / "campaign_openpiton_first_surface_acceptance" / "profiles"
DEFAULT_SELECTION_CONFIG = REPO_ROOT / "config" / "campaign_openpiton_first_surface_acceptance" / "selection.json"
DEFAULT_GATE_JSON = REPO_ROOT / "work" / "campaign_openpiton_first_surface_acceptance_gate.json"
...
def _load_profile(*, profiles_dir: Path, profile_name: str) -> tuple[Path, dict[str, Any]]:
    ...
    if not profile_path.is_file():
        raise FileNotFoundError(
            f"campaign OpenPiton first-surface acceptance profile not found: {profile_path}"
        )
...
def apply_profile(
    *,
    ...
    gate_payload = build_gate(
        xiangshan_status_payload=_read_json(xiangshan_status_json),
        openpiton_gate_payload=_read_json(openpiton_gate_json),
    ...
    return {
        "schema_version": 1,
        "scope": "set_campaign_openpiton_first_surface_acceptance",
...
def main() -> int:
    ...
    payload = apply_profile(
        ...
        openpiton_gate_json=args.openpiton_gate_json.resolve(),
