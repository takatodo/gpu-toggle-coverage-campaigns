#!/usr/bin/env python3
"""
Resolve the currently selected VeeR same-family profile against the current
...
DEFAULT_STEP_JSON = REPO_ROOT / "work" / "campaign_veer_same_family_step.json"
DEFAULT_SELECTION_CONFIG = REPO_ROOT / "config" / "campaign_veer_same_family" / "selection.json"
DEFAULT_JSON_OUT = REPO_ROOT / "work" / "campaign_veer_same_family_gate.json"
...
def _resolve_profile_payload(
    *,
    ...
    if not profile_path.is_file():
        raise FileNotFoundError(f"campaign VeeR same-family profile not found: {profile_path}")
...
def _build_outcome(*, profile_payload: dict[str, Any], step_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if step_design and selected_design and step_design != selected_design:
        return {
            ...
            "next_action": "recompute_veer_same_family_step_or_fix_profile",
    ...
    if step_mode == "default_gate_hold":
        if step_status in {
            ...
            "veer_eh2_default_gate_unresolved",
        }:
            return {
                "status": "default_gate_hold",
                "reason": "selected_profile_keeps_VeeR-EH2_on_the_current_unresolved_default_gate",
        ...
        if step_status == "ready_to_accept_veer_eh2_default_gate":
            return {
                "status": "default_gate_ready",
                "reason": "the_selected_VeeR_same-family_surface_already_has_a_ready_default_gate_line",
                "next_action": "accept_veer_eh2_default_gate_line",
                "selected_design": step_design or selected_design or None,
        ...
        return {
            ...
            "next_action": step_outcome.get("next_action") or "repair_veer_eh2_default_gate_line",
            "selected_design": step_design or selected_design or None,
        }

    if step_mode == "candidate_only_threshold":
        if step_status == "decide_veer_eh2_candidate_only_vs_new_default_gate" and (
            best_ready_threshold_value == candidate_threshold_value
        ):
            return {
                "status": "candidate_only_ready",
                "reason": "selected_profile_accepts_the_checked_in_VeeR-EH2_candidate_only_hybrid_win",
                "next_action": "accept_veer_eh2_candidate_only_step",
                "selected_design": step_design or selected_design or None,
        ...
        return {
            ...
            "reason": "selected_profile_requires_a_checked_in_VeeR_candidate-only_line_that_is_not_currently_available",
            "next_action": step_outcome.get("next_action") or "repair_veer_eh2_candidate_only_line",
            "selected_design": step_design or selected_design or None,
            "candidate_threshold_value": candidate_threshold_value,
        }
    ...
    return {
        ...
        "next_action": "fix_campaign_veer_same_family_profile",
    }
...
def build_gate(*, step_payload: dict[str, Any], selection_payload: dict[str, Any], selection_path: Path) -> dict[str, Any]:
    ...
    return {
        "schema_version": 1,
        "scope": "campaign_veer_same_family_gate",
        ...
        "context": {
            "accepted_veer_first_surface_status": step_context.get("accepted_veer_first_surface_status"),
