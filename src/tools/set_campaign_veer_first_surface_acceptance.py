#!/usr/bin/env python3
"""
Apply a named VeeR first-surface acceptance profile and regenerate the active gate.
"""
...
from audit_campaign_veer_first_surface_acceptance_gate import build_gate
...
DEFAULT_PROFILES_DIR = REPO_ROOT / "config" / "campaign_veer_first_surface_acceptance" / "profiles"
DEFAULT_SELECTION_CONFIG = REPO_ROOT / "config" / "campaign_veer_first_surface_acceptance" / "selection.json"
DEFAULT_GATE_JSON = REPO_ROOT / "work" / "campaign_veer_first_surface_acceptance_gate.json"
...
def apply_profile(
    *,
    ...
    gate_payload = build_gate(
        runtime_gate_payload=_read_json(runtime_gate_json),
        veer_gate_payload=_read_json(veer_gate_json),
    ...
    return {
        "schema_version": 1,
        "scope": "set_campaign_veer_first_surface_acceptance",
...
def main() -> int:
    ...
    payload = apply_profile(
        ...
        runtime_gate_json=args.runtime_gate_json.resolve(),
        veer_gate_json=args.veer_gate_json.resolve(),
