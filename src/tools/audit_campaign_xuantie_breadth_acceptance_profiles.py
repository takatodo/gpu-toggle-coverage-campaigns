#!/usr/bin/env python3
"""
Evaluate named XuanTie breadth acceptance profiles independently of the current
...
from audit_campaign_xuantie_breadth_acceptance_gate import build_gate
...
DEFAULT_PROFILES_DIR = REPO_ROOT / "config" / "campaign_xuantie_breadth_acceptance" / "profiles"
DEFAULT_SELECTION_CONFIG = REPO_ROOT / "config" / "campaign_xuantie_breadth_acceptance" / "selection.json"
DEFAULT_JSON_OUT = REPO_ROOT / "work" / "campaign_xuantie_breadth_acceptance_profiles.json"
...
def build_profiles_matrix(
    *,
        ...
        profiles.append(
            {
                ...
                "selection": {
                    ...
                    "accept_selected_breadth": profile_payload.get("accept_selected_breadth"),
                    "notes": profile_payload.get("notes"),
                },
                "gate": gate,
            }
    ...
    recommended_decision_axis = "repair_xuantie_breadth_acceptance_profiles"
    if recommended_profile_name is not None:
        if current_profile_classification == "ready" and current_profile_name == recommended_profile_name:
            recommended_decision_axis = "accept_current_xuantie_breadth_acceptance_profile"
        else:
            recommended_decision_axis = "choose_named_xuantie_breadth_acceptance_profile"

    return {
        "schema_version": 1,
        "scope": "campaign_xuantie_breadth_acceptance_profiles",
