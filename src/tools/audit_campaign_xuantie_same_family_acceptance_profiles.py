#!/usr/bin/env python3
"""
Evaluate named XuanTie same-family acceptance profiles independently of the
...
from audit_campaign_xuantie_same_family_acceptance_gate import build_gate
...
DEFAULT_PROFILES_DIR = REPO_ROOT / "config" / "campaign_xuantie_same_family_acceptance" / "profiles"
DEFAULT_SELECTION_CONFIG = REPO_ROOT / "config" / "campaign_xuantie_same_family_acceptance" / "selection.json"
DEFAULT_JSON_OUT = REPO_ROOT / "work" / "campaign_xuantie_same_family_acceptance_profiles.json"
...
def build_profiles_matrix(
    *,
        ...
        profiles.append(
            {
                ...
                "selection": {
                    ...
                    "accept_selected_same_family_step": profile_payload.get("accept_selected_same_family_step"),
                    "notes": profile_payload.get("notes"),
                },
                "gate": gate,
            }
    ...
    recommended_decision_axis = "repair_xuantie_same_family_acceptance_profiles"
    if recommended_profile_name is not None:
        if current_profile_classification == "ready" and current_profile_name == recommended_profile_name:
            recommended_decision_axis = "accept_current_xuantie_same_family_acceptance_profile"
        else:
            recommended_decision_axis = "choose_named_xuantie_same_family_acceptance_profile"

    return {
        "schema_version": 1,
        "scope": "campaign_xuantie_same_family_acceptance_profiles",
