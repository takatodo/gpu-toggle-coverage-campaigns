#!/usr/bin/env python3
"""
Resolve the currently selected VeeR first-surface profile against the current
VeeR first-surface step summary.
"""
...
DEFAULT_STEP_JSON = REPO_ROOT / "work" / "campaign_veer_first_surface_step.json"
DEFAULT_SELECTION_CONFIG = REPO_ROOT / "config" / "campaign_veer_first_surface" / "selection.json"
DEFAULT_JSON_OUT = REPO_ROOT / "work" / "campaign_veer_first_surface_gate.json"
...
def _resolve_profile_payload(
    *,
    ...
    if not profile_path.is_file():
        raise FileNotFoundError(f"campaign VeeR first-surface profile not found: {profile_path}")
...
def _build_outcome(*, profile_payload: dict[str, Any], step_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    best_ready_threshold_value = step_outcome.get("candidate_threshold_value") or step_selected_design.get(
        "threshold5_candidate", {}
    ).get("threshold_value")

    if step_design and selected_design and step_design != selected_design:
        return {
            "status": "blocked_selected_design_mismatch",
            "reason": "selected_profile_targets_a_different_VeeR_design_than_the_current_step_artifact",
            "next_action": "recompute_veer_first_surface_step_or_fix_profile",
    ...
    if step_mode == "default_gate_hold":
        if step_status in {
            ...
            "veer_eh1_default_gate_unresolved",
        }:
            return {
                "status": "default_gate_hold",
                "reason": "selected_profile_keeps_VeeR-EH1_on_the_current_unresolved_default_gate",
        ...
        if step_status == "ready_to_accept_veer_eh1_default_gate":
            return {
                "status": "default_gate_ready",
                "reason": "the_selected_VeeR_surface_already_has_a_ready_default_gate_line",
                "next_action": "accept_veer_eh1_default_gate_line",
                "selected_design": step_design or selected_design or None,
        ...
        return {
            ...
            "reason": "selected_profile_requests_default_gate_progress_but_the_current_step_artifact_does_not_support_it",
            "next_action": step_outcome.get("next_action") or "repair_veer_eh1_default_gate_line",
            "selected_design": step_design or selected_design or None,
        }

    if step_mode == "candidate_only_threshold":
        if step_status == "decide_veer_eh1_candidate_only_vs_new_default_gate" and (
            best_ready_threshold_value == candidate_threshold_value
        ):
            return {
                "status": "candidate_only_ready",
                "reason": "selected_profile_accepts_the_checked_in_VeeR-EH1_candidate_only_hybrid_win",
                "next_action": "accept_veer_eh1_candidate_only_step",
                "selected_design": step_design or selected_design or None,
                "comparison_path": step_outcome.get("candidate_comparison_path")
                or step_selected_design.get("threshold5_candidate", {}).get("path"),
                "speedup_ratio": step_outcome.get("speedup_ratio")
                or step_selected_design.get("threshold5_candidate", {}).get("speedup_ratio"),
                "candidate_threshold_value": candidate_threshold_value,
            }
        return {
            "status": "candidate_only_blocked",
            "reason": "selected_profile_requires_a_checked_in_VeeR_candidate_only_line_that_is_not_currently_available",
            "next_action": step_outcome.get("next_action") or "repair_veer_candidate_only_line",
            "selected_design": step_design or selected_design or None,
            "candidate_threshold_value": candidate_threshold_value,
        }
    ...
    return {
        ...
        "next_action": "fix_campaign_veer_first_surface_profile",
    }
...
def build_gate(*, step_payload: dict[str, Any], selection_payload: dict[str, Any], selection_path: Path) -> dict[str, Any]:
    ...
    return {
        "schema_version": 1,
        "scope": "campaign_veer_first_surface_gate",
        ...
        "context": {
            "active_runtime_profile_name": step_context.get("active_runtime_profile_name"),
            "active_runtime_status": step_context.get("active_runtime_status"),
            "recommended_first_design": step_context.get("recommended_first_design"),
            "fallback_design": step_context.get("fallback_design"),
        },
        "outcome": outcome,
    }
