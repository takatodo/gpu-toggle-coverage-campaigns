#!/usr/bin/env python3
"""
Apply a named XuanTie same-family acceptance profile and regenerate the active gate.
"""
...
DEFAULT_GATE_JSON = REPO_ROOT / "work" / "campaign_xuantie_same_family_acceptance_gate.json"
...
def apply_profile(
    *,
    profile_name: str,
    profiles_dir: Path,
    branch_gate_json: Path,
    same_family_gate_json: Path,
    selection_path: Path,
    gate_json_path: Path,
) -> dict[str, Any]:
    ...
    gate_payload = build_gate(
        branch_gate_payload=_read_json(branch_gate_json),
        same_family_gate_payload=_read_json(same_family_gate_json),
    ...
    return {
        "schema_version": 1,
        "scope": "set_campaign_xuantie_same_family_acceptance",
...
def main() -> int:
    ...
    payload = apply_profile(
        ...
        branch_gate_json=args.branch_gate_json.resolve(),
        same_family_gate_json=args.same_family_gate_json.resolve(),
