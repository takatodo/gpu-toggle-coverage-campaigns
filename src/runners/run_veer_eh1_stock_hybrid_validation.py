...
DEFAULT_MDIR = ROOT_DIR / "work" / "vl_ir_exp" / "veer_eh1_gpu_cov_vl"
DEFAULT_TEMPLATE = ROOT_DIR / "config" / "slice_launch_templates" / "veer_eh1.json"
DEFAULT_JSON_OUT = ROOT_DIR / "output" / "validation" / "veer_eh1_stock_hybrid_validation.json"
DEFAULT_PROGRAM_HEX = (
    ROOT_DIR / "third_party" / "rtlmeter" / "designs" / "VeeR-EH1" / "tests" / "hello" / "program.hex"
)
TOP_MODULE = "veer_eh1_gpu_cov_tb"
...
def _build_validation_payload(
    *,
    ...
    host_probe_extra_defines: list[str],
    flow_cmd: list[str],
    ...
    reference_gate = {
        ...
        "description": (
            "A VeeR-EH1 single-surface candidate passes when the stock-Verilator timed-TB host probe and GPU replay "
    ...
    payload: dict[str, Any] = {
        ...
        "inputs": {
            ...
            "program_hex": str(args.program_hex.resolve()),
            ...
            "host_probe_extra_defines": host_probe_extra_defines,
        },
        "artifacts": {
            ...
            "runtime_program_hex": str(runtime_inputs["program_hex"]),
            "runtime_program_entries_bin": str(runtime_inputs["program_entries_bin"]),
        },
        "commands": {
            "flow": flow_cmd,
        },
        ...
        "caveats": [
            ...
            "VeeR gpu_cov hidden preload is materialized through program_entries.bin before host probing",
        ],
    }
...
def main(argv: list[str]) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Run the stock-Verilator hybrid single-surface validation for VeeR-EH1."
    )
    ...
    args.program_hex = args.program_hex.resolve()
    args.json_out = args.json_out.resolve()
    args.flow_json_out = (args.flow_json_out or (mdir / "veer_eh1_host_gpu_flow_summary.json")).resolve()
    args.host_report_out = (args.host_report_out or (mdir / "veer_eh1_host_probe_report.json")).resolve()
    args.host_state_out = (args.host_state_out or (mdir / "veer_eh1_host_init_state.bin")).resolve()
    args.final_state_out = (args.final_state_out or (mdir / "veer_eh1_gpu_final_state.bin")).resolve()
    runtime_inputs = ensure_runtime_program_entries(mdir=mdir, program_hex=args.program_hex)
    host_probe_extra_defines = veer_program_entries_probe_defines(TOP_MODULE)
    ...
    flow_cmd = [
        ...
        "--program-entries-bin",
        str(runtime_inputs["program_entries_bin"]),
    ]
    for raw_define in host_probe_extra_defines:
        flow_cmd.append(f"--host-probe-extra-define={raw_define}")
    for override in args.host_set:
