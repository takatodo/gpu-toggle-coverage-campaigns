...
DEFAULT_JSON_OUT = ROOT_DIR / "output" / "validation" / "veer_eh1_cpu_baseline_validation.json"
DEFAULT_BINARY_NAME = "veer_eh1_cpu_baseline_probe"
DEFAULT_PROGRAM_HEX = (
...
def _load_probe_json(text: str) -> dict[str, Any] | None:
    if not text.strip():
        return None
    start = text.find("{")
    if start < 0:
...
def _build_probe_binary(
    ...
    *,
    ...
    for raw_define in extra_defines:
        cmd.append(f"--extra-define={raw_define}")
...
def _run_probe_binary(
    ...
    *,
    ...
    cmd = [
        ...
        "--program-entries-bin",
        str(program_entries_bin),
    ]
    for name in sorted(settings):
        cmd.extend(["--set", f"{name}={settings[name]}"])
    start = time.perf_counter()
    proc = subprocess.run(cmd, text=True, capture_output=True, check=False, cwd=binary_path.parent)
...
def _build_validation_payload(
    *,
    ...
    payload: dict[str, Any] = {
        ...
        "caveats": [
            "CPU baseline measures the VeeR-EH1 host probe binary wall clock after build; compile time is excluded from v1.",
            "This baseline reuses the stock-Verilator timed-TB single-surface candidate and does not imply design-visible GPU deltas.",
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(
        description="Run the stock-Verilator CPU baseline validation for VeeR-EH1."
    )
    ...
    args.host_report_out = (args.host_report_out or (mdir / "veer_eh1_cpu_baseline_host_report.json")).resolve()
    ...
    build_cmd, build_proc = _build_probe_binary(
        mdir, args.template, args.binary_out, extra_defines=host_probe_extra_defines
    )
    flow_cmd: list[str] = []
    ...
    if build_proc.returncode == 0:
        flow_cmd, flow_proc, elapsed_ms = _run_probe_binary(
            ...
            settings=settings,
            program_entries_bin=Path(runtime_inputs["program_entries_bin"]),
        )
        host_report = _load_probe_json(flow_proc.stdout)
