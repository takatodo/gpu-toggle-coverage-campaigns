...
DEFAULT_MDIR = ROOT_DIR / "work" / "vl_ir_exp" / "tlul_fifo_async_vl"
DEFAULT_TEMPLATE = ROOT_DIR / "config" / "slice_launch_templates" / "tlul_fifo_async.json"
DEFAULT_JSON_OUT = ROOT_DIR / "output" / "validation" / "tlul_fifo_async_stock_hybrid_validation.json"
...
def _build_validation_payload(
    *,
    ...
    reference_gate = {
        ...
        "description": (
            "A tlul_fifo_async campaign reference surface passes when the generic timed-TB host probe and GPU replay "
...
def main(argv: list[str]) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Run the stock-Verilator hybrid campaign-reference validation for tlul_fifo_async."
    )
    ...
    parser.add_argument("--host-post-reset-cycles", type=int, default=16)
    parser.add_argument("--host-set", action="append", default=[], metavar="FIELD=VALUE")
    parser.add_argument("--campaign-threshold-bits", type=int, default=35)
    ...
    args.flow_json_out = (args.flow_json_out or (mdir / "tlul_fifo_async_host_gpu_flow_summary.json")).resolve()
    args.host_report_out = (args.host_report_out or (mdir / "tlul_fifo_async_host_probe_report.json")).resolve()
    args.host_state_out = (args.host_state_out or (mdir / "tlul_fifo_async_host_init_state.bin")).resolve()
    args.final_state_out = (args.final_state_out or (mdir / "tlul_fifo_async_gpu_final_state.bin")).resolve()
