...
from xuantie_c9xx_runtime_inputs import ensure_runtime_pat_pair


SCRIPT_DIR = Path(__file__).resolve().parent
ROOT_DIR = SCRIPT_DIR.parent.parent
FLOW_TOOL = ROOT_DIR / "src" / "tools" / "run_tlul_slice_host_gpu_flow.py"
DEFAULT_MDIR = ROOT_DIR / "work" / "vl_ir_exp" / "xuantie_c906_gpu_cov_gate_vl"
DEFAULT_TEMPLATE = ROOT_DIR / "config" / "slice_launch_templates" / "xuantie_c906.json"
DEFAULT_JSON_OUT = ROOT_DIR / "output" / "validation" / "xuantie_c906_stock_hybrid_validation.json"
DEFAULT_INST_PAT = (
    ROOT_DIR / "third_party" / "rtlmeter" / "designs" / "XuanTie-C906" / "tests" / "hello" / "inst.pat"
)
DEFAULT_DATA_PAT = (
    ROOT_DIR / "third_party" / "rtlmeter" / "designs" / "XuanTie-C906" / "tests" / "hello" / "data.pat"
)
...
def _template_defaults(path: Path) -> tuple[int, int]:
    ...
    return (int(args.get("gpu_nstates", 8)), int(args.get("gpu_sequential_steps", 56)))
...
def _build_validation_payload(
    *,
    ...
    runtime_pat_paths: dict[str, Path],
    flow_cmd: list[str],
    ...
    reference_gate = {
        ...
        "target_support_tier": "candidate_non_opentitan_single_surface",
        "description": (
            "A XuanTie-C906 single-surface candidate passes when the stock-Verilator timed-TB host probe and GPU replay "
            "agree on the checked-in outputs, rsp_queue_overflow_o remains zero, and replay retains nonzero toggle coverage."
        ),
    ...
    if isinstance(metrics, dict):
        ...
        metrics["campaign_wall_time_ms"] = (
            float(campaign_wall_time_ms) if isinstance(campaign_wall_time_ms, (int, float)) else None
        )
    payload: dict[str, Any] = {
        ...
        "clock_ownership": str(
            (flow_json or {}).get("clock_ownership")
            or (host_report or {}).get("clock_ownership")
            or "tb_timed_coroutine"
        ),
        "acceptance_gate": REFERENCE_GATE_NAME,
        "inputs": {
            "mdir": str(args.mdir.resolve()),
            "template": str(args.template.resolve()),
            "inst_pat": str(args.inst_pat.resolve()),
            "data_pat": str(args.data_pat.resolve()),
        ...
        "artifacts": {
            ...
            "runtime_inst_pat": str(runtime_pat_paths["inst.pat"]),
            "runtime_data_pat": str(runtime_pat_paths["data.pat"]),
        },
        "commands": {
            "flow": flow_cmd,
        },
        ...
        "performance": metrics,
        "caveats": [
            "candidate single-surface validation accepts stable host-probe-to-gpu output equivalence under tb_timed_coroutine ownership",
            ...
            "hybrid replay sanitizes host-only internal fields before GPU upload because the timed TB stores host-only runtime bytes",
        ],
    }
...
def main(argv: list[str]) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Run the stock-Verilator hybrid single-surface validation for XuanTie-C906."
    )
    ...
    parser.add_argument("--inst-pat", type=Path, default=DEFAULT_INST_PAT)
    parser.add_argument("--data-pat", type=Path, default=DEFAULT_DATA_PAT)
    ...
    args.inst_pat = args.inst_pat.resolve()
    args.data_pat = args.data_pat.resolve()
    args.json_out = args.json_out.resolve()
    args.flow_json_out = (args.flow_json_out or (mdir / "xuantie_c906_host_gpu_flow_summary.json")).resolve()
    args.host_report_out = (args.host_report_out or (mdir / "xuantie_c906_host_probe_report.json")).resolve()
    args.host_state_out = (args.host_state_out or (mdir / "xuantie_c906_host_init_state.bin")).resolve()
    args.final_state_out = (args.final_state_out or (mdir / "xuantie_c906_gpu_final_state.bin")).resolve()
    runtime_pat_paths = ensure_runtime_pat_pair(mdir=mdir, inst_pat=args.inst_pat, data_pat=args.data_pat)
    ...
    for override in list(args.host_set):
        flow_cmd.extend(["--host-set", override])
    ...
    payload = _build_validation_payload(
        args=args,
        runtime_pat_paths=runtime_pat_paths,
        flow_cmd=flow_cmd,
        flow_json=flow_json,
