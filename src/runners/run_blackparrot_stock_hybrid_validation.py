...
DEFAULT_MDIR = ROOT_DIR / "work" / "vl_ir_exp" / "blackparrot_gpu_cov_vl"
DEFAULT_TEMPLATE = ROOT_DIR / "config" / "slice_launch_templates" / "blackparrot.json"
DEFAULT_JSON_OUT = ROOT_DIR / "output" / "validation" / "blackparrot_stock_hybrid_validation.json"
DEFAULT_PROG_MEM = (
    ROOT_DIR / "third_party" / "rtlmeter" / "designs" / "BlackParrot" / "tests" / "hello" / "prog.mem"
)
DEFAULT_TARGET = "blackparrot"
DEFAULT_TOP_MODULE = "blackparrot_gpu_cov_tb"
...
def _template_defaults(path: Path) -> tuple[int, int]:
    ...
    args = runner_args(payload)
    return (int(args.get("gpu_nstates", 8)), int(args.get("gpu_sequential_steps", 56)))


def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(
        description="Run the stock-Verilator hybrid single-surface validation for BlackParrot."
    )
    parser.add_argument("--mdir", type=Path, default=DEFAULT_MDIR)
    parser.add_argument("--template", type=Path, default=DEFAULT_TEMPLATE)
    parser.add_argument("--prog-mem", type=Path, default=DEFAULT_PROG_MEM)
    ...
    prog_mem = args.prog_mem.resolve()
    json_out = args.json_out.resolve()

    template_payload = load_template(template_path)
    default_nstates, default_steps = _template_defaults(template_path)
    nstates = args.nstates if args.nstates is not None else default_nstates
