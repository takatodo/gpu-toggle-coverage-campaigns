...
DEFAULT_MDIR = ROOT_DIR / "work" / "vl_ir_exp" / "xbar_main_vl"
DEFAULT_TEMPLATE = ROOT_DIR / "config" / "slice_launch_templates" / "xbar_main.json"
DEFAULT_JSON_OUT = ROOT_DIR / "output" / "validation" / "xbar_main_stock_hybrid_validation.json"
...
def _build_validation_payload(
    *,
    ...
    reference_gate = {
        ...
        "description": (
            "An xbar_main campaign reference surface passes when the timed-TB host probe and GPU replay "
    ...
    payload: dict[str, Any] = {
        ...
        "caveats": [
            ...
            "hybrid replay sanitizes host-only internal fields (__VdlySched tail bytes and vlNamep) before GPU upload because timed TB state contains host pointers",
            "multi-clock timed TBs are tracked as reference surfaces as long as the checked-in outputs remain stable across host probe and GPU replay",
        ],
    }
...
def main(argv: list[str]) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Run the stock-Verilator hybrid campaign-reference validation for xbar_main."
    )
    ...
    parser.add_argument("--host-post-reset-cycles", type=int, default=96)
    ...
    args.flow_json_out = (args.flow_json_out or (mdir / "xbar_main_host_gpu_flow_summary.json")).resolve()
    args.host_report_out = (args.host_report_out or (mdir / "xbar_main_host_probe_report.json")).resolve()
    args.host_state_out = (args.host_state_out or (mdir / "xbar_main_host_init_state.bin")).resolve()
    args.final_state_out = (args.final_state_out or (mdir / "xbar_main_gpu_final_state.bin")).resolve()
