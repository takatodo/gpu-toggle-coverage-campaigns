...
DEFAULT_MDIR = ROOT_DIR / "work" / "vl_ir_exp" / "xuantie_e906_gpu_cov_gate_vl"
DEFAULT_TEMPLATE = ROOT_DIR / "config" / "slice_launch_templates" / "xuantie_e906.json"
DEFAULT_JSON_OUT = ROOT_DIR / "output" / "validation" / "xuantie_e906_stock_hybrid_validation.json"
DEFAULT_CASE_PAT = (
    ROOT_DIR / "third_party" / "rtlmeter" / "designs" / "XuanTie-E906" / "tests" / "cmark" / "case.pat"
)
...
def _ensure_case_pat(mdir: Path, source: Path) -> Path:
...
def _build_validation_payload(
    *,
    ...
    reference_gate = {
        ...
        "description": (
            "A XuanTie-E906 single-surface candidate passes when the stock-Verilator timed-TB host probe and GPU replay "
    ...
    payload: dict[str, Any] = {
        ...
        "inputs": {
            ...
            "case_pat": str(args.case_pat.resolve()),
        ...
        "artifacts": {
            ...
            "runtime_case_pat": str(case_pat_path),
        },
        "commands": {
            "flow": flow_cmd,
        },
...
def main(argv: list[str]) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Run the stock-Verilator hybrid single-surface validation for XuanTie-E906."
    )
    ...
    parser.add_argument("--case-pat", type=Path, default=DEFAULT_CASE_PAT)
    ...
    args.case_pat = args.case_pat.resolve()
    args.json_out = args.json_out.resolve()
    args.flow_json_out = (args.flow_json_out or (mdir / "xuantie_e906_host_gpu_flow_summary.json")).resolve()
    args.host_report_out = (args.host_report_out or (mdir / "xuantie_e906_host_probe_report.json")).resolve()
    args.host_state_out = (args.host_state_out or (mdir / "xuantie_e906_host_init_state.bin")).resolve()
    args.final_state_out = (args.final_state_out or (mdir / "xuantie_e906_gpu_final_state.bin")).resolve()
    case_pat_path = _ensure_case_pat(mdir, args.case_pat)
