...
DEFAULT_MDIR = ROOT_DIR / "work" / "vl_ir_exp" / "xuantie_e902_gpu_cov_gate_vl"
DEFAULT_TEMPLATE = ROOT_DIR / "config" / "slice_launch_templates" / "xuantie_e902.json"
DEFAULT_JSON_OUT = ROOT_DIR / "output" / "validation" / "xuantie_e902_stock_hybrid_validation.json"
DEFAULT_CASE_PAT = (
    ROOT_DIR / "third_party" / "rtlmeter" / "designs" / "XuanTie-E902" / "tests" / "hello" / "case.pat"
)
...
def _ensure_case_pat(mdir: Path) -> Path:
    source = DEFAULT_CASE_PAT.resolve()
...
def _build_validation_payload(
    *,
    ...
    reference_gate = {
        ...
        "description": (
            "A XuanTie-E902 single-surface candidate passes when the stock-Verilator timed-TB host probe and GPU replay "
...
def main(argv: list[str]) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Run the stock-Verilator hybrid single-surface validation for XuanTie-E902."
    )
    ...
    args.flow_json_out = (args.flow_json_out or (mdir / "xuantie_e902_host_gpu_flow_summary.json")).resolve()
    args.host_report_out = (args.host_report_out or (mdir / "xuantie_e902_host_probe_report.json")).resolve()
    args.host_state_out = (args.host_state_out or (mdir / "xuantie_e902_host_init_state.bin")).resolve()
    args.final_state_out = (args.final_state_out or (mdir / "xuantie_e902_gpu_final_state.bin")).resolve()
    case_pat_path = _ensure_case_pat(mdir)
    ...
    for raw in args.host_set:
        flow_cmd.extend(["--host-set", raw])
    ...
    return 0 if proc.returncode == 0 and payload["status"] == "ok" else 1
