...
DEFAULT_MDIR = ROOT_DIR / "work" / "vl_ir_exp" / "xuantie_c910_gpu_cov_gate_vl"
DEFAULT_TEMPLATE = ROOT_DIR / "config" / "slice_launch_templates" / "xuantie_c910.json"
DEFAULT_JSON_OUT = ROOT_DIR / "output" / "validation" / "xuantie_c910_stock_hybrid_validation.json"
DEFAULT_INST_PAT = (
    ROOT_DIR / "third_party" / "rtlmeter" / "designs" / "XuanTie-C910" / "tests" / "hello" / "inst.pat"
)
DEFAULT_DATA_PAT = (
    ROOT_DIR / "third_party" / "rtlmeter" / "designs" / "XuanTie-C910" / "tests" / "hello" / "data.pat"
)
...
def _build_validation_payload(
    *,
    ...
    reference_gate = {
        ...
        "description": (
            "A XuanTie-C910 single-surface candidate passes when the stock-Verilator timed-TB host probe and GPU replay "
...
def main(argv: list[str]) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Run the stock-Verilator hybrid single-surface validation for XuanTie-C910."
    )
    ...
    args.flow_json_out = (args.flow_json_out or (mdir / "xuantie_c910_host_gpu_flow_summary.json")).resolve()
    args.host_report_out = (args.host_report_out or (mdir / "xuantie_c910_host_probe_report.json")).resolve()
    args.host_state_out = (args.host_state_out or (mdir / "xuantie_c910_host_init_state.bin")).resolve()
    args.final_state_out = (args.final_state_out or (mdir / "xuantie_c910_gpu_final_state.bin")).resolve()
