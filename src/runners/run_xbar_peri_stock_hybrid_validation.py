...
DEFAULT_JSON_OUT = ROOT_DIR / "output" / "validation" / "xbar_peri_stock_hybrid_validation.json"
...
def _build_validation_payload(
    *,
    ...
    reference_gate = {
        ...
        "description": (
            "An xbar_peri campaign reference surface passes when the timed-TB host probe and GPU replay "
...
def main(argv: list[str]) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Run the stock-Verilator hybrid campaign-reference validation for xbar_peri."
    )
    ...
    args.flow_json_out = (args.flow_json_out or (mdir / "xbar_peri_host_gpu_flow_summary.json")).resolve()
    args.host_report_out = (args.host_report_out or (mdir / "xbar_peri_host_probe_report.json")).resolve()
    args.host_state_out = (args.host_state_out or (mdir / "xbar_peri_host_init_state.bin")).resolve()
    args.final_state_out = (args.final_state_out or (mdir / "xbar_peri_gpu_final_state.bin")).resolve()
