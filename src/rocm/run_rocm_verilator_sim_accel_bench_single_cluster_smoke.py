...
DEFAULT_OUT_DIR = Path("/tmp/rocm_verilator_sim_accel_single_cluster_smoke_v1")
DEFAULT_JSON = SCRIPT_DIR / "rocm_verilator_sim_accel_bench_single_cluster_smoke.json"
DEFAULT_MD = SCRIPT_DIR / "rocm_verilator_sim_accel_bench_single_cluster_smoke.md"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run or summarize a ROCm-bridge verilator_sim_accel_bench single-cluster smoke "
            "and write canonical artifacts."
        )
    )
...
def _write_summary_md(path: Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        "# ROCm verilator_sim_accel_bench Single-Cluster Smoke",
        "",
        ...
        f"- hybrid_cluster_index: `{bench.get('hybrid_cluster_index')}`",
        f"- hybrid_mismatch: `{bench.get('hybrid_mismatch')}`",
        f"- mismatch: `{bench.get('mismatch')}`",
        f"- compact_mismatch: `{bench.get('compact_mismatch')}`",
        f"- hybrid_gpu_ms_per_rep: `{bench.get('hybrid_gpu_ms_per_rep')}`",
        f"- hybrid_cpu_ms_per_rep: `{bench.get('hybrid_cpu_ms_per_rep')}`",
...
def main() -> int:
    ...
    env = _rocm_env()
    wrapper_log = out_dir / "wrapper_single_cluster_smoke.log"
    ...
    pass_run = (
        bench_metrics.get("hybrid_mode") == "single-cluster"
        and bench_metrics.get("hybrid_mismatch") == 0
        and bench_metrics.get("mismatch") == 0
        and bench_metrics.get("compact_mismatch") == 0
    )

    payload: dict[str, Any] = {
        "schema_version": "rocm-verilator-sim-accel-single-cluster-smoke-v1",
        "pass": bool(pass_run),
        ...
        "next_blocker": (
            "promote_rocm_single_cluster_into_bundle_and_runner_validation"
            if pass_run
            else "fix_rocm_single_cluster_wrapper_smoke"
        ),
