...
DEFAULT_OUT_DIR = Path("/tmp/rocm_tlul_fifo_sync_runner_single_cluster_smoke_v1")
DEFAULT_JSON = SCRIPT_DIR / "rocm_mainline_runner_single_cluster_smoke.json"
DEFAULT_MD = SCRIPT_DIR / "rocm_mainline_runner_single_cluster_smoke.md"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run or summarize a ROCm mainline OpenTitan slice single-cluster smoke and "
    ...
    parser.add_argument("--hybrid-cluster-index", type=int, default=0)
...
def _detect_seq_partition_gap(out_dir: Path) -> bool:
    raw_kernel = out_dir / "generated_dir_cache" / TEMPLATE.stem / "raw" / f"{TEMPLATE.stem}_gpu_cov_tb.sim_accel.kernel.cu"
...
def _write_md(path: Path, payload: dict[str, Any]) -> None:
    ...
    metrics = smoke.get("metrics") or {}
    collector_status = ((smoke.get("collector") or {}).get("status") or {})
    coverage = ((smoke.get("collector") or {}).get("coverage") or {})
    fallback_metrics = dict(payload.get("baseline_metrics") or {})
    target = smoke.get("target", "") or payload.get("requested_target", "")
    backend = ((smoke.get("gpu_execution_backend") or {}).get("selected") or payload.get("requested_backend") or "")
    hybrid_mode = metrics.get("hybrid_mode", fallback_metrics.get("hybrid_mode"))
    hybrid_cluster_index = metrics.get("hybrid_cluster_index", fallback_metrics.get("hybrid_cluster_index"))
    mismatch = collector_status.get("mismatch", fallback_metrics.get("mismatch"))
    compact_mismatch = collector_status.get("compact_mismatch", fallback_metrics.get("compact_mismatch"))
    hybrid_mismatch = metrics.get("hybrid_mismatch", fallback_metrics.get("hybrid_mismatch"))
    points_hit = coverage.get("coverage_points_hit", fallback_metrics.get("points_hit"))
    points_total = coverage.get("coverage_points_total", fallback_metrics.get("points_total"))
    gpu_ms_per_rep = metrics.get("gpu_ms_per_rep", fallback_metrics.get("gpu_ms_per_rep"))
    hybrid_gpu_ms_per_rep = metrics.get("hybrid_gpu_ms_per_rep", fallback_metrics.get("hybrid_gpu_ms_per_rep"))
    lines = [
        "# ROCm Mainline Runner Single-Cluster Smoke",
        "",
        f"- status: `{'pass' if payload['pass'] else 'fail'}`",
        f"- target: `{target}`",
        f"- gpu_execution_backend: `{backend}`",
        f"- hybrid_mode: `{hybrid_mode}`",
        f"- hybrid_cluster_index: `{hybrid_cluster_index}`",
        f"- mismatch: `{mismatch}`",
        ...
        f"- hybrid_gpu_ms_per_rep: `{hybrid_gpu_ms_per_rep}`",
...
def main() -> int:
    ...
    pass_run = bool(
        ...
        and runner_run.get("returncode") == 0
    ...
    if (baseline_metrics.get("hybrid_mismatch") or 0) != 0 and _detect_seq_partition_gap(out_dir):
        semantic_gap_classification = "cluster_interface_missing_seq_commit"
    structured_gap_soft_accepted = bool(structured_gap.get("soft_accepted"))
    next_blocker = "fix_rocm_mainline_runner_single_cluster_smoke"
    ...
    elif "cluster_count=0" in baseline_text:
        next_blocker = "select_cluster_sane_mainline_single_cluster_target"
    elif "the operation cannot be performed in the present state" in baseline_text:
        next_blocker = "fix_rocm_mainline_runner_single_cluster_launch_error"
    elif structured_gap_soft_accepted or semantic_gap_classification == "cluster_interface_missing_seq_commit":
        next_blocker = "single_cluster_semantic_gap_missing_seq_commit"
    elif (_extract_int_metric(baseline_text, "hybrid_mismatch") or 0) != 0:
        next_blocker = "fix_rocm_mainline_runner_single_cluster_hybrid_mismatch"
    bundle_dir_match = re.search(r"^bundle_dir=(.+)$", str(runner_run.get("stdout") or ""), re.MULTILINE)
    bundle_dir = bundle_dir_match.group(1).strip() if bundle_dir_match else ""
    payload: dict[str, Any] = {
        "schema_version": "rocm-mainline-runner-single-cluster-smoke-v1",
        "pass": pass_run,
        ...
        "baseline_log_excerpt": baseline_text[-4000:],
        ...
        "paths": {
            ...
            "build_log": str((Path(smoke_summary.get("bundle_dir") or bundle_dir) / "build_hipcc.log").resolve()),
        },
    }
