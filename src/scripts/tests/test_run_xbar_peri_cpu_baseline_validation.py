...
MODULE_PATH = SCRIPT_DIR.parent / "runners" / "run_xbar_peri_cpu_baseline_validation.py"
...
class RunXbarPeriCpuBaselineValidationTest(unittest.TestCase):
    def setUp(self) -> None:
        self.module = _load_module("run_xbar_peri_cpu_baseline_validation_test", MODULE_PATH)
    ...
    def test_runner_writes_cpu_baseline_validation_json(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            def _fake_run(cmd: list[str], **kwargs):
                ...
                self.assertIn("cfg_access_ack_data_pct_i=98", argv)
                self.assertNotIn("access_ack_data_pct=98", argv)
                return mock.Mock(
                    returncode=0,
                    stdout=json.dumps(
                        {
            ...
            argv = [
                "run_xbar_peri_cpu_baseline_validation.py",
                "--mdir",
                str(mdir),
                "--template",
                str(template),
            ...
            self.assertEqual(payload["campaign_measurement"]["steps_executed"], 184)
