...
MODULE_PATH = SCRIPT_DIR.parent / "runners" / "run_xbar_peri_stock_hybrid_validation.py"
...
class RunXbarPeriStockHybridValidationTest(unittest.TestCase):
    def setUp(self) -> None:
        self.module = _load_module("run_xbar_peri_stock_hybrid_validation_test", MODULE_PATH)
    ...
    def test_runner_writes_campaign_reference_validation_json(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            def _fake_run(cmd: list[str], **kwargs):
                    ...
                    json.dumps(
                        {
                            ...
                            "outputs": {
                                "done_o": 0,
                                "cfg_signature_o": 4294963371,
                                "host_req_accepted_o": 22,
                                "device_req_accepted_o": 22,
                                "device_rsp_accepted_o": 22,
                                "host_rsp_accepted_o": 22,
                                "rsp_queue_overflow_o": 0,
                                "progress_cycle_count_o": 88,
                                "progress_signature_o": 0,
                                "toggle_bitmap_word0_o": 3158899464,
                                "toggle_bitmap_word1_o": 904021982,
                                "toggle_bitmap_word2_o": 10026897,
                            },
                            "changed_watch_field_count": 0,
                            "state_delta": {
                                "changed_byte_count": 32,
                                "changed_known_field_count": 0,
                                "first_changed_offsets": [58],
                            ...
                            "campaign_timing": {
                                ...
                                "wall_time_ms": 1.458,
                                "per_run_wall_time_ms": [1.458],
                                "gpu_kernel_time_complete": True,
                                "gpu_kernel_time_ms_total": 1.419264,
                                "per_run_gpu_kernel_time_ms": [1.419264],
                            },
                            "gpu_runs": [
                                {
                                    "index": 1,
                ...
                return mock.Mock(
                    ...
                    stdout="\n".join(
                        [
                            "device 0: Demo GPU",
                            "ok: steps=56 kernels_per_step=1 patches_per_step=0 grid=1 block=256 nstates=32 storage=2880 B",
                            "gpu_kernel_time_ms: total=1.419264  per_launch=0.025344  (CUDA events, kernels only; HtoD patches before launch excluded)",
                            "gpu_kernel_time: per_state=0.792 us  (per_launch / nstates)",
                            "wall_time_ms: 1.458  (host; one GPU sync unless RUN_VL_HYBRID_SYNC_EACH_STEP=1)",
                            '{"target":"xbar_peri"}',
                        ]
                    )
                    + "\n",
            ...
            argv = [
                "run_xbar_peri_stock_hybrid_validation.py",
                "--mdir",
                str(mdir),
            ...
            self.assertEqual(payload["campaign_measurement"]["wall_time_ms"], 1.458)
