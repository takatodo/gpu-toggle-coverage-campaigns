...
MODULE_PATH = SCRIPT_DIR.parent / "runners" / "run_tlul_request_loopback_cpu_baseline_validation.py"
...
class RunTlulRequestLoopbackCpuBaselineValidationTest(unittest.TestCase):
    def setUp(self) -> None:
        self.module = _load_module("run_tlul_request_loopback_cpu_baseline_validation_test", MODULE_PATH)
    ...
    def test_runner_writes_cpu_baseline_validation_json(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            template.write_text(
                json.dumps({"runner_args_template": {"driver_defaults": {"cfg_req_valid_pct_i": 96}}}) + "\n",
                encoding="utf-8",
            )
            ...
            def _fake_run(cmd: list[str], **kwargs):
                ...
                self.assertIn("cfg_req_valid_pct_i=96", argv)
                self.assertIn("cfg_rsp_valid_pct_i=92", argv)
                return mock.Mock(
                    returncode=0,
                    stdout=json.dumps(
                        {
            ...
            argv = [
                "run_tlul_request_loopback_cpu_baseline_validation.py",
                "--mdir",
                str(mdir),
                "--template",
                str(template),
            ...
            self.assertEqual(payload["coverage"]["bits_hit"], 2)
            self.assertEqual(
            ...
            self.assertEqual(payload["campaign_measurement"]["bits_hit"], 2)
            ...
            self.assertEqual(payload["outputs"]["progress_cycle_count_o"], 5)
