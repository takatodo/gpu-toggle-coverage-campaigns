...
MODULE_PATH = SCRIPT_DIR.parent / "runners" / "run_tlul_sink_time_to_threshold_comparison.py"
...
class RunTlulSinkTimeToThresholdComparisonTest(unittest.TestCase):
    def setUp(self) -> None:
        self.module = _load_module("run_tlul_sink_time_to_threshold_comparison_test", MODULE_PATH)
    ...
    def test_runner_rejects_threshold_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self._write_artifact(
                hybrid,
                _artifact(backend="stock_verilator_hybrid", threshold_value=6),
            )
