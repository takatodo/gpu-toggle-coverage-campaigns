...
MODULE_PATH = SCRIPT_DIR.parent / "runners" / "run_tlul_err_cpu_baseline_validation.py"
...
class RunTlulErrCpuBaselineValidationTest(unittest.TestCase):
    def setUp(self) -> None:
        self.module = _load_module("run_tlul_err_cpu_baseline_validation_test", MODULE_PATH)
    ...
    def test_runner_writes_cpu_baseline_validation_json(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            argv = [
                "run_tlul_err_cpu_baseline_validation.py",
                "--mdir",
                str(mdir),
                "--template",
                str(template),
            ...
            self.assertEqual(payload["coverage"]["bits_hit"], 9)
            self.assertEqual(
            ...
            self.assertEqual(payload["campaign_measurement"]["bits_hit"], 9)
