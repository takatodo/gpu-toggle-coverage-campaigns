...
MODULE_PATH = SCRIPT_DIR.parent / "runners" / "run_tlul_err_stock_hybrid_validation.py"
...
class RunTlulErrStockHybridValidationTest(unittest.TestCase):
    def setUp(self) -> None:
        self.module = _load_module("run_tlul_err_stock_hybrid_validation_test", MODULE_PATH)
    ...
    def test_runner_writes_campaign_reference_validation_json(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            def _fake_run(cmd: list[str], **kwargs):
                    ...
                    json.dumps(
                        {
                            ...
                            "outputs": {
                                "done_o": 0,
                                "cfg_signature_o": 3775,
                                "host_req_accepted_o": 1,
                                "device_req_accepted_o": 1,
                                "device_rsp_accepted_o": 0,
                                "host_rsp_accepted_o": 0,
                                "rsp_queue_overflow_o": 0,
                                "progress_cycle_count_o": 2,
                                "progress_signature_o": 1920145081,
                                "toggle_bitmap_word0_o": 511,
                            ...
                            "campaign_timing": {
                                ...
                                "wall_time_ms": 1.5,
                                "per_run_wall_time_ms": [1.5],
                                "gpu_kernel_time_complete": True,
                                "gpu_kernel_time_ms_total": 0.42,
                                "per_run_gpu_kernel_time_ms": [0.42],
                            },
                            "gpu_runs": [
                                {
                                    "index": 1,
                ...
                return mock.Mock(
                    ...
                    stdout="\n".join(
                        [
                            "device 0: Demo GPU",
                            "ok: steps=56 kernels_per_step=1 patches_per_step=0 grid=1 block=256 nstates=32 storage=832 B",
                            "gpu_kernel_time_ms: total=0.420000  per_launch=0.420000  (CUDA events, kernels only; HtoD patches before launch excluded)",
                            "gpu_kernel_time: per_state=13.125 us  (per_launch / nstates)",
            ...
            argv = [
                "run_tlul_err_stock_hybrid_validation.py",
                "--mdir",
                str(mdir),
            ...
            self.assertEqual(payload["target"], "tlul_err")
            ...
            self.assertEqual(payload["outputs"]["cfg_signature_o"], 3775)
            self.assertEqual(payload["outputs"]["rsp_queue_overflow_o"], 0)
            self.assertEqual(payload["toggle_coverage"]["bits_hit"], 9)
            self.assertEqual(
            ...
            self.assertEqual(payload["campaign_measurement"]["wall_time_ms"], 1.5)
            self.assertEqual(payload["campaign_measurement"]["steps_executed"], 56)
            self.assertEqual(payload["performance"]["device_name"], "Demo GPU")
            self.assertEqual(payload["performance"]["campaign_wall_time_ms"], 1.5)
            self.assertEqual(payload["performance"]["campaign_timing"]["per_run_wall_time_ms"], [1.5])
            self.assertEqual(
    ...
    def test_runner_reports_reference_gate_blocker_when_outputs_diverge(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            def _fake_run(cmd: list[str], **kwargs):
                    ...
                    json.dumps(
                        {
                            ...
                            "outputs": {
                                ...
                                "host_rsp_accepted_o": 0,
                                "rsp_queue_overflow_o": 0,
                                "progress_cycle_count_o": 3,
                                "progress_signature_o": 1920145082,
                                "toggle_bitmap_word0_o": 511,
                                "toggle_bitmap_word1_o": 0,
                                "toggle_bitmap_word2_o": 0,
                            },
                            "changed_watch_field_count": 0,
                            "campaign_timing": {"wall_time_ms": 1.5},
                            "gpu_runs": [{"index": 1, "steps": 56, "performance": {"wall_time_ms": 1.5}}],
                        }
                    )
                    + "\n",
                    encoding="utf-8",
                ...
                return mock.Mock(returncode=0, stdout="device 0: Demo GPU\n", stderr="")
