...
MODULE_PATH = SCRIPT_DIR.parent / "runners" / "run_openpiton_stock_hybrid_validation.py"
...
class RunOpenPitonStockHybridValidationTest(unittest.TestCase):
    def setUp(self) -> None:
        self.module = _load_module("run_openpiton_stock_hybrid_validation_test", MODULE_PATH)
    ...
    def test_runner_delegates_to_enrollment_common(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(ensure_mock.call_args.kwargs["runtime_input_format"], "bin")
            self.assertEqual(ensure_mock.call_args.kwargs["runtime_input_patch_bytes"][0]["offset"], 1664)
