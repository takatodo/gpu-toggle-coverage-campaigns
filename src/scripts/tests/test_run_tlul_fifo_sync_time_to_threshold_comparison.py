...
MODULE_PATH = SCRIPT_DIR.parent / "runners" / "run_tlul_fifo_sync_time_to_threshold_comparison.py"
...
class RunTlulFifoSyncTimeToThresholdComparisonTest(unittest.TestCase):
    def setUp(self) -> None:
        self.module = _load_module("run_tlul_fifo_sync_time_to_threshold_comparison_test", MODULE_PATH)
    ...
    def test_runner_writes_hybrid_win_comparison_json(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self._write_artifact(
                baseline,
                _artifact(backend="stock_verilator_cpu_baseline", wall_time_ms=40.0, steps_executed=2),
            )
            self._write_artifact(
                hybrid,
                _artifact(backend="stock_verilator_hybrid", wall_time_ms=10.0, steps_executed=2),
            )
            ...
            self.assertEqual(payload["baseline"]["campaign_measurement"]["steps_executed"], 2)
            self.assertEqual(payload["hybrid"]["campaign_measurement"]["steps_executed"], 2)

    def test_runner_rejects_threshold_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            baseline = root / "baseline.json"
            hybrid = root / "hybrid.json"
            ...
            self._write_artifact(
                hybrid,
                _artifact(backend="stock_verilator_hybrid", threshold_value=4),
            )
