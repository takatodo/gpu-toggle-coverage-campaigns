...
MODULE_PATH = SCRIPT_DIR.parent / "runners" / "run_caliptra_time_to_threshold_comparison.py"
...
class RunCaliptraTimeToThresholdComparisonTest(unittest.TestCase):
    def setUp(self) -> None:
        self.module = _load_module("run_caliptra_time_to_threshold_comparison_test", MODULE_PATH)
    ...
    def test_runner_writes_hybrid_win_comparison_json(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self._write_artifact(
                baseline,
                _artifact(backend="stock_verilator_cpu_baseline", wall_time_ms=12.0),
            )
            self._write_artifact(
                hybrid,
                _artifact(backend="stock_verilator_hybrid", wall_time_ms=3.0),
            )
            ...
            self.assertEqual(payload["target"], "caliptra")
