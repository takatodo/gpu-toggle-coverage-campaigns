...
MODULE_PATH = SCRIPT_DIR.parent / "runners" / "run_tlul_sink_stock_hybrid_validation.py"
...
class RunTlulSinkStockHybridValidationTest(unittest.TestCase):
    def setUp(self) -> None:
        self.module = _load_module("run_tlul_sink_stock_hybrid_validation_test", MODULE_PATH)
    ...
    def test_runner_writes_campaign_reference_validation_json(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
                    ...
                    json.dumps(
                        {
                            ...
                            "outputs": {
                                ...
                                "toggle_bitmap_word0_o": 1811939328,
                            ...
                            "campaign_timing": {
                                ...
                                "wall_time_ms": 1.25,
                                "per_run_wall_time_ms": [1.25],
                                "gpu_kernel_time_complete": True,
                                "gpu_kernel_time_ms_total": 0.35,
                                "per_run_gpu_kernel_time_ms": [0.35],
                            },
                            "gpu_runs": [
                                {
                                    "index": 1,
                ...
                return mock.Mock(
                    ...
                    stdout="\n".join(
                        [
                            ...
                            "gpu_kernel_time_ms: total=0.350000  per_launch=0.350000  (CUDA events, kernels only; HtoD patches before launch excluded)",
                            "gpu_kernel_time: per_state=10.937 us  (per_launch / nstates)",
                            "wall_time_ms: 2.500  (host; one GPU sync unless RUN_VL_HYBRID_SYNC_EACH_STEP=1)",
                            '{"target":"tlul_sink"}',
                        ]
                    )
                    + "\n",
            ...
            argv = [
                "run_tlul_sink_stock_hybrid_validation.py",
                "--mdir",
                str(mdir),
            ...
            self.assertEqual(payload["target"], "tlul_sink")
            ...
            self.assertEqual(payload["outputs"]["cfg_signature_o"], 264)
            self.assertEqual(payload["outputs"]["rsp_queue_overflow_o"], 0)
            self.assertEqual(payload["toggle_coverage"]["bits_hit"], 5)
            self.assertEqual(
            ...
            self.assertEqual(payload["campaign_measurement"]["wall_time_ms"], 1.25)
            self.assertEqual(payload["campaign_measurement"]["steps_executed"], 56)
            self.assertEqual(payload["performance"]["device_name"], "Demo GPU")
            self.assertEqual(payload["performance"]["campaign_wall_time_ms"], 1.25)
            self.assertEqual(payload["performance"]["campaign_timing"]["per_run_wall_time_ms"], [1.25])
            self.assertEqual(
                    ...
                    json.dumps(
                        {
                            ...
                            "outputs": {
                                ...
                                "progress_signature_o": 2,
                                "toggle_bitmap_word0_o": 1811939328,
                                "toggle_bitmap_word1_o": 0,
                                "toggle_bitmap_word2_o": 1,
                            },
                            "changed_watch_field_count": 0,
                            "campaign_timing": {"wall_time_ms": 1.25},
                            "gpu_runs": [{"index": 1, "steps": 56, "performance": {"wall_time_ms": 1.25}}],
                        }
                    )
                    + "\n",
                    encoding="utf-8",
