...
MODULE_PATH = SCRIPT_DIR.parent / "runners" / "run_tlul_request_loopback_time_to_threshold_comparison.py"
...
def _artifact(
    *,
    ...
    steps_executed: int = 12,
    status: str = "ok",
    target: str = "tlul_request_loopback",
) -> dict[str, object]:
    return {
        "schema_version": 1,
        "status": status,
...
class RunTlulRequestLoopbackTimeToThresholdComparisonTest(unittest.TestCase):
    def setUp(self) -> None:
        self.module = _load_module("run_tlul_request_loopback_time_to_threshold_comparison_test", MODULE_PATH)
    ...
    def test_runner_writes_hybrid_win_comparison_json(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self._write_artifact(
                baseline,
                _artifact(backend="stock_verilator_cpu_baseline", wall_time_ms=40.0),
            )
            self._write_artifact(
                hybrid,
    ...
    def test_runner_rejects_threshold_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self._write_artifact(
                hybrid,
                _artifact(backend="stock_verilator_hybrid", threshold_value=3),
            )
